...
0\& --- \& 3fff:1::/32 \& push 24 \& 2 \\
0 \& --- \& 3fff:2::/32 \& push 27 \& 2 \\
2 \& 9 \& --- \& pop \& 0\\
...
\begin{frame}
    \frametitle{MPLS tunnel applications}
    \begin{itemize}
    \item example: ISP selling connection from building A to building B
        \begin{itemize}
        \item make it appear like `Ethernet connection'
        \item make it follow fixed path that has enough bandwidth/low latency
        \end{itemize}
    \item example: traffic engineering (next slide)
        \begin{itemize}
        \item customizing routing complex changes to OSPF/etc. metrics
        \item recall: hard to express complex preferences in OSPF `distances'
        \end{itemize}
    \end{itemize}
\end{frame}
