...
\foreach \x/\y in {read response/write request,write response/read request,accept/accept} {
    \path[myline,dotted] ([xshift=0.5cm]\x.south west) -- ++ (0cm,-0.25cm) -| ([yshift=.25cm,xshift=-.5cm]\y.north west)
...
\begin{pgfonlayer}{bg}
    \node[fill=blue!10,fit=(read request) (write response) (close server),
          label={south:spawn new process (fork) or thread per conn.}] {}
\end{pgfonlayer}
\end{tikzpicture}
\end{frame}
