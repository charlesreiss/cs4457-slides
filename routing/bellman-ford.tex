...
\frametitle{all-pairs Bellman-Ford (2)}
    \begin{itemize}
        ...
        \begin{itemize}
        \item for each node X:
        \item \myemph<2>{if $c + d(B, X) < d(A, X)$}, \\
        \item then \myemph<3>{$d(A,X) \leftarrow c+d(B,X)$, $p(A,X) = B$}
        \end{itemize}
    \vspace{.5cm}
    \item<2->
        \myemph<2>{if A$\rightarrow$B$\rightarrow$X faster than current A$\rightarrow$X route} \\
        \myemph<2>{then choose it over current route}
    \item<3->
        \myemph<3>{when choosing: update distance A$\rightarrow$X + next link for A$\rightarrow$X}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{running Bellman-Ford}
\begin{tikzpicture}
\tikzset{
