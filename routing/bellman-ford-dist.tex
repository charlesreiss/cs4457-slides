...
\providecommand{\noroute}{$\infty$/---}
\providecommand{\mymark}[2]{\alt<#1->{\myemph<#1>{#2}}{}}
\begin{frame}[fragile]{distributing Bellman-Ford [start]}
\begin{tikzpicture}
\tikzset{
    computer/.style={inner sep=0mm,outer sep=0mm,execute at begin node={\computer}},
    switch/.style={inner sep=0mm,outer sep=0mm,execute at begin node={\switch}},
...
\matrix[anchor=north east,one dv,
    ...
    A \& B \& C \& D \\
    \noroute \& B/0 \& \only<1-5>{\noroute}\mymark{5}{7/C} \& \only<1-5>{\noroute}\mymark{5}{2/D} \\
};
\begin{visibleenv}<3-4>
\draw[msg link] (C) -- (D)
    node[midway,above,msg data] {I'm C, costs: C=0};
\end{visibleenv}
\begin{visibleenv}<5-6>
\draw[msg link] (D) -- (B)
    node[midway,above,msg data] {I'm D, costs: C=5,D=0};
\end{visibleenv}
\end{scope}
\end{tikzpicture}
\end{frame}
