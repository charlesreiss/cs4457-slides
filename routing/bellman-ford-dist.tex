...
\begin{itemize}
\item \myemph<1>{on each node A run:}
\item repeatedly: 
\item for each link \myemph{from A} (going to B, distance c)
    \begin{itemize}
    ...
    \end{itemize}
\vspace{.5cm}
\item<2-> \myemph<2>{on node A: track $d(A, X)$, $p(A, X)$}
\item<3-> \myemph<3>{node A needs to get $d(B, X)$ to run algorithm}
\end{itemize}
\end{frame}
