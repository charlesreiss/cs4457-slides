...
\matrix[anchor=south,one dv,
    ...
    A \& B \& C \& D \\
    0/A \& 2/B \& 4/C \& \only<1-2>{3/B}\only<3->{\infty/---} \\
};
...
\begin{visibleenv}<2>
\draw[msg link] (A) -- (B)
    node[midway,above left,msg data] { I'm A: A=0,B=2,C=4,D=\sout{3}{\large \infty} (poison reverse)};
\end{visibleenv}
\begin{visibleenv}<3>
\draw[msg link] (B) -- (A)
    node[midway,above left,msg data] { I'm B: A=4,B=0,C=2,D=\infty };
\end{visibleenv}
\end{scope}
\end{tikzpicture}
\end{frame}
